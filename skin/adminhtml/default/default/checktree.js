...
//					console.log(lists[ul].getElementsByTagName('input').length);
					var lists2=lists[ul].getElementsByTagName('input');
//					console.log(lists2[0].checked);
					for(var li=0;li<lists2.length;li++){
						if(lists2[li].checked){
				...
				for(var li=0;li<childNodes.length;li++){
					for(var tag=0;tag<childNodes[li].childNodes.length;tag++){
						...
						if(child.tagName&&child.type&&child.tagName.match(/^input/i)&&child.type.match(/^checkbox/i)){
							...
							allChecked&=child.checked;if(child.checked)boxCount++
							}
